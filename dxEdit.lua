...
function dxCreateEditBox (name, text, placeholder, posX, posY, w, h, font, color, masked, maxLength)
    ...
    outputDebugString("dxEditBox created - ID: "..#editTable.."; Name: "..name.."", 0, 0, 200, 20)
    return #editTable
end
...
addEventHandler("onClientRender", root, 
	function()
		for k, v in pairs(editTable) do
			...
			dxDrawRectangle(v["x"], v["y"], v["w"], v["h"], tocolor(0, 0, 0, 200), true)

			dxDrawRectangle(v["x"], v["y"], v["w"], v["h"], tocolor(0, 0, 0, v["designAlpha"]), true)
			dxDrawLinedRectangle(v["x"], v["y"], v["w"], v["h"], tocolor(v["activeColor"][1], v["activeColor"][2], v["activeColor"][3], v["designAlpha"]/75*200), 1, true)
...
function getValidKey (key)
	...
	if (validKeys[string.lower(key)]) then
		...
		return false --Жоский костыль, т.к. английиские буквы капсом не писались(
	end
end
...
function dxDestroyEditBox (id)
	...
	outputDebugString("dxEditBox destroyed - ID: "..id.."", 0, 0, 200, 20)
	return true
end
